...
class InstagramAPI:
    ...
    def uploadPhoto(self, photo, caption=None, upload_id=None, is_sidecar=None):
        ...
        data = {'upload_id': upload_id,
                ...
                'photo': ('pending_media_%s.jpg' % upload_id, urllib.request.urlopen(photo), 'application/octet-stream', {'Content-Transfer-Encoding': 'binary'})}
        if is_sidecar:
            data['is_sidecar'] = '1'
    ...
    def tagFeed(self, tag):
        userFeed = self.SendRequest('feed/tag/' + tag + '/?rank_token=' + str(self.rank_token) + '&ranked_content=true&')
        return userFeed
    ...
    def searchTags(self, query):
        query = self.SendRequest('tags/search/?is_typeahead=true&q=' + query + '&rank_token=' + str(self.rank_token))
        return query
